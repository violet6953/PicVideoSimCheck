...
Usage:
    python build/build-gpu.py                    # 标准构建（打包 exe）
    python build/build-gpu.py --version 1.0.4    # 指定版本号
    python build/build-gpu.py --no-clean         # 跳过清理步骤（增量构建）

Features:
    - 自动版本号同步到 .spec 和 installer.iss
    - 构建信息注入（时间戳、Git Commit）
    - 构建产物输出到 PicSimProcess\\dist-gpu\\PicSimProcess\\
    - 构建产物大小统计

Note:
    本脚本只负责 PyInstaller 打包 exe，不自动调用 Inno Setup。
    安装包请手动使用 Inno Setup 编译 build\\installer-gpu.iss，
    输出文件参考: Output\\PicSimProcess_GPU_Setup_v{VERSION}.exe
"""
...
DIST_DIR = PROJECT_ROOT / "dist-gpu"      # PyInstaller 输出根目录
SPEC_FILE = BUILD_DIR / "PicSimProcess-GPU.spec"
INSTALLER_ISS = BUILD_DIR / "installer-gpu.iss"
OUTPUT_DIR = PROJECT_ROOT / "Output"        # 手动安装包输出目录
...
def inject_build_info(version: str) -> Path:
    ...
    crt_time = datetime.now().strftime("%Y-%m-%d")
...
def clean() -> None:
    ...
    dirs_to_remove = [
        ...
        DIST_DIR,  # PyInstaller output folder: dist-gpu/
...
def print_summary(version: str, duration_sec: float) -> None:
    ...
    print(f"\n  Output:      {exe_dir}")
    print(f"  Next step:   手动使用 Inno Setup 编译 {INSTALLER_ISS.name}")
    print(f"               安装包输出参考: {OUTPUT_DIR}\\PicSimProcess_GPU_Setup_v{version}.exe")
    print("=" * 60)
...
Examples:
  python build/build-gpu.py                    # 标准构建（打包 exe）
  python build/build-gpu.py -v 1.0.4           # 指定版本号
  python build/build-gpu.py --no-clean         # 跳过清理步骤（增量构建）
        """.strip(),
    )
    parser.add_argument(
        "-v", "--version",
        default=None,
        help="Version number (e.g., 1.0.4). Auto-detected from installer.iss if omitted.",
    )
    parser.add_argument(
        "-n", "--no-clean",
        action="store_true",
...
def main() -> int:
    ...
    print_summary(version, duration)
